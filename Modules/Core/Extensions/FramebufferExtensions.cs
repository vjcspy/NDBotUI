...
namespace NDBotUI.Modules.Core.Extensions
{
    public static class FramebufferExtensions
    {
        ...
        private static Mat ToEmguMat(this in FramebufferHeader header, byte[] buffer, bool convertToGray = false)
        {
            ...
            int channels = (int)header.Bpp / 8; // Số kênh màu (3 cho RGB, 4 cho RGBA)
            ...
            if (header.Bpp == 8) // Nếu đã là ảnh grayscale
            {
                ...
                try
                {
                    using Mat wrappedMat = new Mat(height, width, DepthType.Cv8U, 1, handle.AddrOfPinnedObject(), width);
            ...
            else
            {
                ...
                try
                {
                    colorMat = new Mat(height, width, DepthType.Cv8U, channels, handle.AddrOfPinnedObject(), width * channels);
                }
                finally
                {
                    handle.Free();
