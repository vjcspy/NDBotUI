...
public static class FrameBufferSkiaExtensions
{
    ...
    /// <returns>A <see cref="SKBitmap"/> that represents the image contained in the frame buffer, or <see langword="null"/>
    ...
    public static SKBitmap? ToSKBitmap(this in FramebufferHeader header, byte[] buffer)
    {
        ...
        SKColorType colorType = header.StandardizePixelFormat(ref buffer, out SKAlphaType alphaType);
    ...
    /// <summary>
    /// Returns the <see cref="SKColorType"/> that describes pixel format of an image that is stored according to the information
    /// present in this <see cref="FramebufferHeader"/>. Because the <see cref="SKColorType"/> enumeration does not allow for all
    ...
    /// <returns>A <see cref="SKColorType"/> that describes how the image data is represented in this <paramref name="buffer"/>.</returns>
