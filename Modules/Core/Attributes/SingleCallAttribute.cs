...
public class SingleCallAttribute : Attribute
{
    ...
    {
        if (CalledMethods.ContainsKey(method)) return false; // Method đã được gọi trước đó

        if (!method.IsStatic) throw new InvalidOperationException("SingleCallAttribute only supports static methods");
