...
public class ReRollHomeStep1Effect : EffectBase
{
    ...
    protected override bool Filter(EventAction action)
    {
        if (action.Payload is BaseActionPayload baseActionPayload)
        {
            ...
            if (gameInstance is { } gameInstanceData)
            {
                ...
                return currentStatus == EteReRollStatus.Step1HomeScreen;
            }
        }
    ...
    {
        Logger.Info(">>Process ReRollHomeStep1Effect");
        ...
        switch (detectTemplatePoint.TemplateKey)
        {
            ...
            case EteTemplateKey.GuideSupplyQuick:
            {
                await emulatorConnection.ClickPPointAsync(new PPoint(82.0f, 42.7f));
                isClicked = true;
                break;
            }

            case EteTemplateKey.SupplyBannerStandard:
            {
                await emulatorConnection.ClickPPointAsync(new PPoint(10.3f, 45.5f));
                isClicked = true;
                break;
            }

            case EteTemplateKey.GuideSupplyFree:
            {
                await emulatorConnection.ClickPPointAsync(new PPoint(71.3f, 91.5f));
                isClicked = true;
                break;
            }
            case EteTemplateKey.GuideSupplyBack:
            {
                await emulatorConnection.ClickPPointAsync(new PPoint(6.8f, 4.7f));
                isClicked = true;
                break;
            }
